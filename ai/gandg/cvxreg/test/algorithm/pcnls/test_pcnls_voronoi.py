...
from ai.gandg.cvxreg.algorithm.pcnls.pcnls_voronoi import PCNLSVoronoiEstimator
...
class TestPCNLSVoronoiEstimator(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        set_random_seed(19)
        ...
        cls.pcnlsv1 = PCNLSVoronoiEstimator()
        cls.model1 = cls.pcnlsv1.train(cls.X, cls.y)
    ...
    def test_weights_shape(self):
        self.assertEqual(self.model1.weights.shape, (6, 3))

    def test_in_sample_risk(self):
        yhat1 = self.pcnlsv1.predict(self.model1, self.X)
        self.assertAlmostEqual(
            np.round(np.sum(np.square(yhat1 - self.y)) / len(self.y), decimals=4), 0.3644)

    def test_out_of_sample_error(self):
        yhat1_test = self.pcnlsv1.predict(self.model1, self.X_test)
        self.assertAlmostEqual(
            np.round(np.sum(np.square(yhat1_test - self.y_test)) / len(self.y_test), decimals=4),
            0.3653)
