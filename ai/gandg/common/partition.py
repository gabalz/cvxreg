...
def find_min_dist_centers(data, partition, dist=squared_distance, op=np.max, batch_size=1000):
    ...
    if dist in (euclidean_distance, squared_distance):
        metric = 'sqeuclidean'
        take_sqrt = (op != np.max and dist == euclidean_distance)
    else:
        metric = None
        take_sqrt = False
    ...
    for cell in partition.cells:
        ...
        for start in range(0, n, batch_size):
            block = pts[start:min(start + batch_size, n)]
            if metric:
                dblock = distance.cdist(block, pts, metric)
            else:
                dblock = dist(block, pts)
            if take_sqrt:
                np.sqrt(dblock, out=dblock)
            scores = op(dblock, axis=1)

            local_best = np.argmin(scores)
            if scores[local_best] < best_score:
                best_score = scores[local_best]
                best_idx = cell[start + local_best]
        centers.append(best_idx)
    return tuple(centers)


def cell_radii(data, partition, centers=None, dist=euclidean_distance):
